...
public class FacultyControllerIntegrationTest {
    ...
    @Test
    public void OneFacultyFromDataBaseAreAvailableOnWeb() throws Exception {
        this.mockMvc.perform(get("/faculty/add").accept(MediaType.parseMediaType("text/html;charset=UTF-8")))
                .andExpect(status().isOk()).andExpect(content().contentType("text/html;charset=UTF-8"));
    }

    @Test
    public void givenWacWhenServletContextThenItProvidesFacultyController() {
        ServletContext servletContext = webApplicationContext.getServletContext();
        Assert.assertNotNull(servletContext);
        Assert.assertTrue(servletContext instanceof MockServletContext);
        Assert.assertNotNull(webApplicationContext.getBean("facultyController"));
    }
